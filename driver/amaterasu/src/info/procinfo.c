 ...
 */
PPROC_INFO ProcInfoGet(_PoolType_ PoolType, _In_ PFLT_CALLBACK_DATA Data) {
 ...
 *    -
 */
static NTSTATUS AcquireElevationStatus(_Inout_ PBOOLEAN isElevataed, _In_ PACCESS_TOKEN pToken) {

    NTSTATUS Status;
    TOKEN_ELEVATION eToken;

    Status = SeQueryInformationToken(pToken, TokenElevation, &eToken);
    if(!NT_SUCCESS(Status)) {
        return Status;
    }

    *isElevated = eToken.TokenIsElevated;
 ...
 *    -
 */
static NTSTATUS GetAccessTokenInfo(_Inout_ PPROC_INFO ProcInfo, _In_ PACCESS_TOKEN pToken) {

    NTSTATUS Status;

    Status = AcquireElevationStatus(&ProcInfo->isElevated, pToken);
    if(!NT_SUCCESS(Status)) {
        return Status;
    }
 ...
 *    -
 */
static inline PACCESS_TOKEN GetAccessToken(_In_ PFLT_CALLBACK_DATA Data) {
    ...
    return PsReferencePrimaryToken(Proc);
}
 ...
 *    -
 */
static inline void GetTID(_Inout PULONG TID, _In_ PFLT_CALLBACK_DATA Data) {
...
NTSTATUS ProcInfoInit(_Inout_ PPROC_INFO ProcInfo, _In_ PFLT_CALLBACK_DATA Data) {
    ...
    pToken = GetAccessToken(Data);
    if(!pToken) {
        return STATUS_UNSUCCESSFUL;
    }

    Status = GetAccessTokenInfo(ProcInfo, pToken);
    if(!NT_SUCCESS(Status)) {
        return Status;
    }
