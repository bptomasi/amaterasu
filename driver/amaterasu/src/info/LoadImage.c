...
NTSTATUS GetExtendedInfo(_In_ PLOAD_IMAGE_INFO LoadImageInfo, _In_ PIMAGE_INFO ImageInfo)
{
    ...
    Status = UnicodeStrToWSTR(LoadImageInfo->PoolType, FileName, &LoadImageInfo->FileName, &LoadImageInfo->FileNameSize);
...
NTSTATUS InitLoadImageInfoFields(_In_ PLOAD_IMAGE_INFO LoadImageInfo, _In_ PIMAGE_INFO ImageInfo, _In_ PUNICODE_STRING FullImageName)
{
    ...
    Status = UnicodeStrToWSTR(LoadImageInfo->PoolType, FullImageName, &LoadImageInfo->FullImageName, &LoadImageInfo->FullImageNameSize);
    if (!NT_SUCCESS(Status)) {
        ExFreePoolWithTag(LoadImageInfo->FullImageName, 'wstr');
        return Status;
    }
 ...
 */
PLOAD_IMAGE_INFO LoadImageInfoGet(_In_ POOL_TYPE PoolType, _In_ PIMAGE_INFO ImageInfo, _In_ PUNICODE_STRING FullImageName, _In_ HANDLE PID)
{
    NTSTATUS Status;
    PLOAD_IMAGE_INFO LoadImageInfo;
    ...
    LoadImageInfo->PID = PID;

    Status = LoadImageInfoInit(LoadImageInfo, ImageInfo, FullImageName);
    if (!NT_SUCCESS(Status))
