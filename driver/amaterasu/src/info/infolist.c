 ...
 */
PINFO_LIST InfoListGet(_PoolType_ PoolType, _In_ ULONG MaxRecords) {
...
static void InfoListFreeNodes(_Inout_ PINFO_LIST InfoList) {
    ...
    while(InfoList->RecordsAllocated) {
        Info = InfoListRemoveHead(InfoList); 
        InfoListFree(&Info);
    }
}
...
static void AppendInfo(_Inout_ PINFO_LIST InfoList, _In_ PINFO Info) {
     ...
     *  at the head.
     */
    if(InfoList->RecordsAllocated + 1 >= MaxRecords) {
        Info = InfoListRemovetTail(InfoList);
        InfoFree(&Info);
    } else {
...
NTSTATUS InfoListAppend(_Inout_ PINFO_LIST InfoList, _In_ PFLT_CALLBACK_DATA Data) {
    ...
    Info = InfoGet(InfoList->PoolType, Data);
    if(!Info) {
