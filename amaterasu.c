...
NTSTATUS AmaterasuSetup(PIRP Irp, PIO_STACK_LOCATION IrpIoStack) {
    ...
    DbgPrint("TargetName:  %ws\n", DriverSettings->TargetName);
    line();
    DbgPrint("TargetNameSize: %d\n", DriverSettings->TargetNameSize);
    line();
    DbgPrint("MaxRecords: %d\n", Amaterasu.DriverSettings.MaxRecords);
    line();

    //RtlCopyMemory(&Amaterasu.DriverSettings, DriverSettings, sizeof * DriverSettings);
    //*InfoSize = sizeof * DriverSettings;
    line();
    DbgPrint(" %d\n", __LINE__);
    line();
